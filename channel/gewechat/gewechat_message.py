...
class GeWeChatMessage(ChatMessage):
    def __init__(self, msg, client: GewechatClient):
        ...
        # 确保 self.content 始终被初始化为一个字符串，即使在某些情况下没有内容 
        ...
        if msg_type == 1:  # Text message
            ...
            if not conf().get("speech_recognition", False) and not (self.is_group and conf().get("group_speech_recognition", False)):
                logger.debug(f"[gewechat] voice message ignored because speech_recognition is disabled")
                return
            ...
            self.content = self.msg_data['Content']['string']
            logger.debug(f"[gewechat] Received video message: {self.content}")
        else:
        ...
        if self.is_group:
            ...
            if msg_source:
                try:
                    ...
                    if atuserlist_elem is not None:
                        atuserlist = atuserlist_elem.text or ''  # 如果text为None，使用空字符串
            ...
            # 如果XML解析未确认被at，从PushContent中判断
            if not self.is_at and not xml_parsed:
            ...
            # 如果前两种方式都未确认被at，从消息内容中检查
            if not self.is_at:
                # 获取机器人的昵称
                bot_info_response = self.client.get_brief_info(self.app_id, [self.to_user_id])
                if bot_info_response['ret'] == 200 and bot_info_response['data']:
                    bot_info = bot_info_response['data'][0]
                    bot_nickname = bot_info.get('nickName', '')
                    if bot_nickname:
                        # 检查消息中是否包含 @昵称\u2005
                        at_pattern = f'@{bot_nickname}\u2005'
                        if at_pattern in self.content:
                            self.is_at = True
                            logger.debug(f"[gewechat] Parse is_at from content. self.is_at: {self.is_at}")
            ...
            if bot_info_response['ret'] == 200 and bot_info_response['data']:
                ...
                if bot_nickname:
                    ...
                    at_pattern = f'@{bot_nickname}\u2005'
                    if at_pattern in self.content:
                        self.content = self.content.replace(at_pattern, '')
