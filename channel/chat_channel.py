...
class ChatChannel(Channel):
    ...
    def _generate_reply(self, context: Context, reply: Reply = Reply()) -> Reply:
        ...
        if not e_context.is_pass():
            ...
            if context.type == ContextType.TEXT or context.type == ContextType.IMAGE_CREATE:  # 文字和图片消息
                ...
                memory.USER_VIDEO_CACHE[context["session_id"]] = {
                    "path": context.content,
                    "msg": context.get("msg")
                }
            else:
                ...
                if reply.type == ReplyType.TEXT:
                    ...
                    for i, segment in enumerate(segments):
                        if context.get("isgroup", False):
                            ...
                            segment = conf().get("group_chat_reply_prefix", "") + segment + conf().get(
                                "group_chat_reply_suffix", "")
                        else:
                            segment = conf().get("single_chat_reply_prefix", "") + segment + conf().get(
    ...
    def _send(self, reply: Reply, context: Context):
        try:
            ...
            logger.error(f"[chat_channel] sendMsg error: {str(e)}")
            logger.exception(e)
